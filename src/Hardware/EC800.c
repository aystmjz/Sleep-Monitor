    ...
    for (uint8_t i = 1; i <= Try_Time; i++) {
        ...
        while (!ATcmd_Scan(Ack)) {
            ...
            if (timeout > Wait_Time) {
                if (Error != NULL) Debug_printf(Error);
                timeout = 0;
                break;
            }
        }
...
void Pub_Data(const char *topic)
{
    ...
    cJSON_AddNumberToObject(Data_Json, "Temp", (uint16_t)Data.Temp + ((uint16_t)(Data.Temp * 100) % 100) / 100.0);
    cJSON_AddNumberToObject(Data_Json, "Hum", (uint16_t)Data.Hum + ((uint16_t)(Data.Hum * 100) % 100) / 100.0);
    cJSON_AddNumberToObject(Data_Json, "Light", (uint16_t)Data.Light + ((uint16_t)(Data.Light * 100) % 100) / 100.0);
    cJSON_AddNumberToObject(Data_Json, "eco2", Data.eco2);
    cJSON_AddNumberToObject(Data_Json, "tvoc", Data.tvoc);
    // if (Data.Beep)
    //     cJSON_AddTrueToObject(Data_Json, "Fan");
    // else
    //     cJSON_AddFalseToObject(Data_Json, "Fan");
    // if (Data.Led)
