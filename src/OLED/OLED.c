...
void OLED_WriteData(uint8_t *Data, uint8_t Count)
{
	...
	for (i = 0; i < Count; i ++)
	{
		LCD_WriteByte(Data[i]);	//依次发送Data的每一个数据
	}
	//OLED_I2C_Stop();				//I2C终止
}
...
{
	//{OLED_Printf(120-6*6, 0, 6,"FPS %d", Timer_GetFPS());	}	//显示帧率;解除注释开启

	/*遍历每一页*/
	//LCD_SetAddress(0,0,64,32);
	for (uint8_t i = 0; i < 16; i ++)
	{
		...
		//OLED_WriteData(OLED_DisplayBuf[i], 128);
		LCD_SetAddress(0,(i+14)*8,240,(i+15)*8);
		for(uint8_t j=0;j<240;j++)
		{
			//LCD_SetAddress(j,i*8,j,(i+1)*8);
			for(uint8_t l=0;l<8;l++)
			{
				uint8_t x=l/2;
				if(i%2)x+=4;
				if((OLED_DisplayBuf[i/2][j/2]>>x)&0x01)
				{
					LCD_SendBuff[(l*240+j)*2]=0xff;
					LCD_SendBuff[(l*240+j)*2+1]=0xff;
				//LCD_WriteDATA(WHITE);
				}
				else
				{LCD_SendBuff[(l*240+j)*2]=0x00;
					LCD_SendBuff[(l*240+j)*2+1]=0x00;
					//LCD_WriteDATA(BLACK);
				}
		...
		LCD_DMA_TransferLen(240*8*2);
		LCD_DMA_Waite();
		/*for(uint8_t j=0;j<120;j++)
		{
			LCD_SetAddress(j*2,i*8*2,j*2,(i+1)*8*2);
			for(uint8_t l=0;l<8;l++)
			{
				if((OLED_DisplayBuf[i][j]>>l)&0x01)
				{
					LCD_WriteDATA(WHITE);
					LCD_WriteDATA(WHITE);
				}
			...
			LCD_SetAddress(j*2+1,i*8*2,j*2+1,(i+1)*8*2);
			for(uint8_t l=0;l<8;l++)
			{
				if((OLED_DisplayBuf[i][j]>>l)&0x01)
				{
